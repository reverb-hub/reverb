    ...
    if (path.length <= 0) {
      ...
      if (varSectors.length >= 1) {
        ...
        for (const sector of varSectors) {
          try {
            const route = this.varSectors[sector].resolveSubRoute(
              path,
              method,
              {
                [sector]: subPath,
      ...
      if (this.staticSectors[subPath]) {
        return this.staticSectors[subPath].resolveSubRoute(
          path,
          method,
          pathVariables,
        );
...
export class RouteResolver {
  ...
  private resolveRoutes() {
    this.controllerInstances.forEach((controllerInstance: any) => {
      ...
      const methods = Object.getOwnPropertyNames(
        ...
        return isMethod(controllerInstance.constructor.prototype, property) &&
          (Reflect.getMetadata(
            COMPONENT_TYPE.MAPPING,
            controllerInstance.constructor.prototype[property],
          ) === true);
      });

      methods.forEach((method) => {
        const mappingPath = Reflect.getMetadata(
          PATH_METADATA,
          controllerInstance.constructor.prototype[method],
        );
        ...
        const paramMetadata = Reflect.getMetadata(
          ROUTE_ARGS_METADATA,
          controllerInstance.constructor.prototype.constructor,
          method,
        );
        this.routeMap.addRoute(fullPathSections, methodType, {
          handler: (controllerInstance[method] as (...args: any) => any).bind(
            controllerInstance,
          ),
  ...
  private resolveQueryParameters(
